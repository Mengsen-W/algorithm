/*
 * @Date: 2022-02-22 01:08:34
 * @Author: Mengsen Wang
 * @LastEditors: Mengsen Wang
 * @LastEditTime: 2022-02-22 02:09:39
 */
...
class Solution {
 ...
 public:
  int numberOfGoodSubsets(vector<int> nums) {
    ...
    for (int i = 2; i <= num_max; ++i) {
      ...
      for (int mask = (1 << primes.size()) - 1; mask > 0; --mask) {
        if ((mask & subset) == subset) {
          f[mask] =
              (f[mask] + static_cast<long long>(f[mask ^ subset]) * freq[i]) %
              mod;
        }
      }
    }
    ...
    int ans = 0;
    for (int mask = 1, mask_max = (1 << primes.size()); mask < mask_max;
         ++mask) {
      ans = (ans + f[mask]) % mod;
    }
