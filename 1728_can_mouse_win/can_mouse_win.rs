/*
 * @Date: 2022-05-10 09:25:02
 * @Author: Mengsen Wang
 * @LastEditors: Mengsen Wang
 * @LastEditTime: 2022-05-15 19:24:35
 * @FilePath: /algorithm/1728_can_mouse_win/can_mouse_win.rs
 */
        ...
        for cat in 0..total {
            ...
            for dir in meta.dirs {
                    ...
                    && jump <= meta.mouse_jump
                {
                    ...
                    meta.degrees[next_mouse as usize][next_cat as usize][MOUSE_TURN as usize] += 1;
                    row += dir[0];
                    col += dir[1];
                    jump += 1;
                }
                    ...
                    && jump <= meta.cat_jump
                {
                    ...
                    meta.degrees[next_mouse as usize][next_cat as usize][CAT_TURN as usize] += 1;
                    row += dir[0];
                    col += dir[1];
                    jump += 1;
                }
    ...
    while qu.len() > 0 {
        ...
        for (prev_mouse, prev_cat, prev_turn) in prev_states {
                ...
                == UNKNOWN
            {
                ...
                if can_win {
                    meta.results[prev_mouse as usize][prev_cat as usize][prev_turn as usize][0] =
                        result;
                    meta.results[prev_mouse as usize][prev_cat as usize][prev_turn as usize][1] =
                        moves + 1;
                    qu.push((prev_mouse, prev_cat, prev_turn));
                } else {
                    meta.degrees[prev_mouse as usize][prev_cat as usize][prev_turn as usize] -= 1;
                    if meta.degrees[prev_mouse as usize][prev_cat as usize][prev_turn as usize] == 0
                    {
...
fn main() {
    ...
    {
        let grid = vec!["M.C...F"].iter().map(|s| s.to_string()).collect();
        let cat_jump = 1;
        let mouse_jump = 4;
        assert_eq!(can_mouse_win(grid, cat_jump, mouse_jump), true);
    }
