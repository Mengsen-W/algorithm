 ...
 * @FilePath: /algorithm/cpp/1632_matrix_rank_transform/matrix_rank_transform.cpp
 */
...
class UnionFind {
 ...
 public:
  UnionFind(int m, int n) {
    this->m = m;
    this->n = n;
    this->root = vector<vector<pair<int, int>>>(m, vector<pair<int, int>>(n));
...
class Solution {
 public:
  vector<vector<int>> matrixRankTransform(vector<vector<int>>& matrix) {
    ...
    UnionFind uf(m, n);
    for (int i = 0; i < m; i++) {
      unordered_map<int, vector<pair<int, int>>> num2indexList;
      for (int j = 0; j < n; j++) {
    ...
    for (int i = 0; i < m; i++) {
      unordered_map<int, pair<int, int>> num2index;
      for (int j = 0; j < n; j++) {
...
int main() {
  {
    ...
    vector<vector<int>> ans{{1, 2}, {2, 3}};
    assert(Solution().matrixRankTransform(matrix) == ans);
  }
...

  {
    ...
    vector<vector<int>> ans{{4, 2, 3}, {1, 3, 4}, {5, 1, 6}, {1, 3, 4}};
    assert(Solution().matrixRankTransform(matrix) == ans);
  }

  {
    vector<vector<int>> matrix{{7, 3, 6}, {1, 4, 5}, {9, 8, 2}};
    vector<vector<int>> ans{{5, 1, 4}, {1, 2, 3}, {6, 3, 1}};
    assert(Solution().matrixRankTransform(matrix) == ans);
  }
}