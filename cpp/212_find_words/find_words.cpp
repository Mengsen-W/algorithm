/*
 * @Date: 2021-09-16 08:42:05
 * @Author: Mengsen Wang
 * @LastEditors: Mengsen Wang
 * @LastEditTime: 2021-09-16 08:54:06
 */
 ...
 public:
  ...
  bool dfs(vector<vector<char>>& board, int x, int y, TrieNode* root,
    ...
    for (int i = 0; i < 4; ++i) {
      int nx = x + dirs[i][0];
      int ny = y + dirs[i][1];
      if (nx >= 0 && nx < (int)board.size() && ny >= 0 &&
          ny < (int)board[0].size())
        if (board[nx][ny] != '#') dfs(board, nx, ny, root, res);
    }
    board[x][y] = ch;
    return true;
  }
...
int main() {
  {
    ...
    assert(Solution().findWords(board, words) == ans);
  }
  {
