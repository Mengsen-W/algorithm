...
int main() {
  vector<tuple<int, int, int, vector<int>>> tests{
      {11, 2, 4, {3, 4}},
      {5, 1, 5, {1, 1}},
  };

  for (auto &[n, firstPlayer, secondPlayer, ans] : tests) {
    assert(Solution().earliestAndLatest(n, firstPlayer, secondPlayer) == ans);
  }
}