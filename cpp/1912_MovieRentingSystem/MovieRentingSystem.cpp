...
int main() {
  ...
  assert((movieRentingSystem.search(1) == vector<int>{1, 0, 2}));
  movieRentingSystem.rent(0, 1);
  movieRentingSystem.rent(1, 2);
  assert((movieRentingSystem.report() == vector<vector<int>>{{0, 1}, {1, 2}}));
  movieRentingSystem.drop(1, 2);
  assert((movieRentingSystem.search(2) == vector<int>{0, 1}));
}