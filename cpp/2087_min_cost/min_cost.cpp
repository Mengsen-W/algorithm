...
int main() {
  vector<tuple<vector<int>, vector<int>, vector<int>, vector<int>, int>> tests{
      {{1, 0}, {2, 3}, {5, 4, 3}, {8, 2, 6, 7}, 18},
      {{0, 0}, {0, 0}, {5}, {26}, 0},
  };

  for (auto [startPos, homePos, rowCosts, colCosts, expected] : tests) {
    assert(Solution().minCost(startPos, homePos, rowCosts, colCosts) == expected);
  }
  return 0;
}