...
int main() {
  vector<tuple<string, string, vector<int>, vector<int>>> tests{
      {"babacc", "bcb", {1, 3, 3}, {3, 3, 4}},
      {"abyzz", "aa", {2, 1}, {2, 3}},
  };

  for (auto& [s, queryCharacters, queryIndices, ans] : tests) {
    assert(Solution().longestRepeating(s, queryCharacters, queryIndices) == ans);
  }
}