    ...
    for (int i = 0; i < m; ++i) {
      for (int j = 0; j < n; ++j) {
        ...
        for (int k = i + 2; k < m; k += 2) {
          ...
          ans.put((sum2[lx + 1][ly + 1] - sum2[ux][uy + 2]) + (sum1[rx + 1][ry + 1] - sum1[ux][uy]) +
                  (sum1[dx + 1][dy + 1] - sum1[lx][ly]) + (sum2[dx + 1][dy + 1] - sum2[rx][ry + 2]) -
...
int main() {
  ...
  for (auto& [grid, expected] : tests) {
    assert(Solution().getBiggestThree(grid) == expected);
  }
}