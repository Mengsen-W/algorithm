 ...
 public:
  long long minimumMoves(vector<int>& nums, int k, int maxChanges) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      res = min(res, indexSum[i2 + 1] - indexSum[i + 1] - i * count2 + i * count1 - (indexSum[i + 1] - indexSum[i1]) +
                         2 * maxChanges);
    }
    return res;
  }
...
int main() {
  ...
  for (auto& [nums, k, maxChanges, ans] : tests) {
    assert(Solution().minimumMoves(nums, k, maxChanges) == ans);
  }
}