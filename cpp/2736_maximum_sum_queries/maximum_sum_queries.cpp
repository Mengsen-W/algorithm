 ...
 * @FilePath: /algorithm/cpp/2736_maximum_sum_queries/maximum_sum_queries.cpp
 */
...
int main() {
  vector<tuple<vector<int>, vector<int>, vector<vector<int>>, vector<int>>> tests{
  ...
  for (auto &[nums1, nums2, queries, ans] : tests) {
    assert(Solution().maximumSumQueries(nums1, nums2, queries) == ans);
  }
}