 ...
 * @FilePath: /algorithm/cpp/2532_find_crossing_time/find_crossing_time.cpp
 */
...
int main() {
  vector<tuple<int, int, vector<vector<int>>, int>> testMap{
      {1, 3, vector<vector<int>>{{1, 1, 2, 1}, {1, 1, 3, 1}, {1, 1, 4, 1}}, 6},
      {3, 2, vector<vector<int>>{{1, 9, 1, 8}, {10, 10, 10, 10}}, 50},
  };
  for (auto& [n, k, time, ans] : testMap) {
    assert(Solution().findCrossingTime(n, k, time) == ans);
  }
}