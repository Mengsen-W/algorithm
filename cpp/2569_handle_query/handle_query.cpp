 ...
 * @FilePath: /algorithm/cpp/2569_handle_query/handle_query.cpp
 */
...
class SegTree {
 ...
 public:
  ...
  int sumRange(int left, int right) { return query(1, left, right); }

  void reverseRange(int left, int right) { modify(1, left, right); }
...
int main() {
  vector<tuple<vector<int>, vector<int>, vector<vector<int>>, vector<long long>>> tests{
      {{1, 0, 1}, {0, 0, 0}, {{1, 1, 1}, {2, 1, 0}, {3, 0, 0}}, {3}},
      {{1}, {5}, {{2, 0, 0}, {3, 0, 0}}, {5}},
  };

  for (auto& [nums1, nums2, queries, ans] : tests) {
    assert(Solution().handleQuery(nums1, nums2, queries) == ans);
  }
}