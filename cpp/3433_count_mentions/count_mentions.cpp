...
int main() {
  vector<tuple<int, vector<vector<string>>, vector<int>>> tests{
  ...
  for (auto&& [numberOfUsers, events, expect] : tests) {
    assert(Solution().countMentions(numberOfUsers, events) == expect);
  }
}