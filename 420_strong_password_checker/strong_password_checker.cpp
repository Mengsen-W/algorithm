/*
 * @Date: 2022-04-02 00:39:02
 * @Author: Mengsen Wang
 * @LastEditors: Mengsen Wang
 * @LastEditTime: 2022-04-02 00:42:37
 * @FilePath: /algorithm/420_strong_password_checker/strong_password_checker.cpp
 */
 ...
 public:
  int strongPasswordChecker(string password) {
    ...
    if (n < 6) {
      ...
      // 使用 k % 3 = 1
      // 的组的数量，由剩余的替换次数、组数和剩余的删除次数共同决定
      int use2 = min({replace, rm2, remove / 2});
      replace -= use2;
      remove -= use2 * 2;
      // 由于每有一次替换次数就一定有 3 个连续相同的字符（k / 3
      // 决定），因此这里可以直接计算出使用 k % 3 = 2 的组的数量
