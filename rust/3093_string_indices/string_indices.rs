...
fn main() {
    let tests = vec![
        ...
        (
            vec!["abcdefgh", "poiuygh", "ghghgh"],
            vec!["gh", "acbfgh", "acbfegh"],
            vec![2, 0, 2],
        ),
    ];

    for (words_container, words_query, expected) in tests {
        assert_eq!(
            Solution::string_indices(
                words_container.iter().map(|s| s.to_string()).collect(),
                words_query.iter().map(|s| s.to_string()).collect()
            ),
            expected
        );
    }
