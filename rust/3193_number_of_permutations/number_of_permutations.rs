...
fn main() {
    let tests = vec![
        (3, vec![vec![2, 2], vec![0, 0]], 2),
        (3, vec![vec![2, 2], vec![1, 1], vec![0, 0]], 1),
        (2, vec![vec![0, 0], vec![1, 0]], 1),
    ];

    for (n, requirements, ans) in tests {
        assert_eq!(Solution::number_of_permutations(n, requirements), ans);
    }
}
