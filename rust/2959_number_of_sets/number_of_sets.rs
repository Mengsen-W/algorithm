...
fn main() {
    let tests = vec![
        (3, 5, vec![vec![0, 1, 2], vec![1, 2, 10], vec![0, 2, 10]], 5),
        (
            3,
            5,
            vec![vec![0, 1, 20], vec![0, 1, 10], vec![1, 2, 2], vec![0, 2, 2]],
            7,
        ),
    ];

    for (n, max_distance, road, ans) in tests {
        assert_eq!(Solution::number_of_sets(n, max_distance, road), ans);
    }
}
