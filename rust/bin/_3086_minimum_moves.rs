...
impl Solution {
    ...
    pub fn minimum_moves(nums: Vec<i32>, k: i32, max_changes: i32) -> i64 {
        ...
        for i in 0..n {
            index_sum[(i + 1) as usize] =
                index_sum[i as usize] + (nums[i as usize] as i64) * (i as i64);
        ...
        for i in 0..n {
            ...
            let (count1, count2) = (
                sum[(i + 1) as usize] - sum[i1 as usize],
                sum[(i2 + 1) as usize] - sum[(i + 1) as usize],
            );
            res = std::cmp::min(
                res,
                index_sum[(i2 + 1) as usize] - index_sum[(i + 1) as usize] - (i as i64) * count2
                    + (i as i64) * count1
                    - (index_sum[(i + 1) as usize] - index_sum[i1 as usize])
...
fn main() {
    let tests = vec![
        (vec![1, 1, 0, 0, 0, 1, 1, 0, 0, 1], 3, 1, 3),
        (vec![0, 0, 0, 0], 2, 3, 4),
    ];

    for (nums, k, max_changes, ans) in tests {
