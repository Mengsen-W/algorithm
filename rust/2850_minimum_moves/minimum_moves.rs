...
impl Solution {
    ...
    pub fn next_permutation(arr: &mut Vec<Vec<i32>>) -> bool {
        ...
        for j in (p + 1) as usize..arr.len() {
            if Self::is_less(&arr[p as usize], &arr[j]) {
                q = j as i32;
            }
        }
        Self::swap(arr, p as usize, q as usize);
        let mut i = p + 1;
        let mut j = arr.len() as i32 - 1;
        while i < j {
            Self::swap(arr, i as usize, j as usize);
            i += 1;
            j -= 1;
        }
        true
...
fn main() {
    let tests = vec![
        (vec![vec![1, 3, 0], vec![1, 0, 0], vec![1, 0, 3]], 4),
        (vec![vec![1, 1, 0], vec![1, 1, 1], vec![1, 2, 1]], 3),
    ];

    for (grid, ans) in tests {
        assert_eq!(Solution::minimum_moves(grid), ans);
    }
}
