    ...
    pub fn number_of_stable_arrays(zero: i32, one: i32, limit: i32) -> i32 {
        ...
        for i in 1..=zero as usize {
            for j in 1..=one as usize {
                if i > limit as usize {
                    dp[i][j][0] =
                        dp[i - 1][j][0] + dp[i - 1][j][1] - dp[i - limit as usize - 1][j][1];
                } else {
                ...
                if j > limit as usize {
                    dp[i][j][1] =
                        dp[i][j - 1][1] + dp[i][j - 1][0] - dp[i][j - limit as usize - 1][0];
                } else {
...
fn main() {
    let tests = vec![(1, 1, 2, 2), (1, 2, 1, 1), (3, 3, 2, 14)];

    for (zero, one, limit, ans) in tests {
