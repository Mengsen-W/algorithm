    ...
    pub fn minimum_distance(word: String) -> i32 {
        ...
        for i in 1..n {
            ...
            for j in 0..26 {
                let j_i32 = j as i32;
                dp[i][cur as usize][j] =
                    dp[i][cur as usize][j].min(dp[i - 1][prev as usize][j].saturating_add(d));
                dp[i][j][cur as usize] =
                    dp[i][j][cur as usize].min(dp[i - 1][j][prev as usize].saturating_add(d));

                if prev == j_i32 {
                    for k in 0..26 {
                        let d0 = Self::get_distance(k as i32, cur);
                        dp[i][cur as usize][j] =
                            dp[i][cur as usize][j].min(dp[i - 1][k][j].saturating_add(d0));
                        dp[i][j][cur as usize] =
                            dp[i][j][cur as usize].min(dp[i - 1][j][k].saturating_add(d0));
                    }
                }
            }
        }
...
fn main() {
    let tests = vec![("CAKE", 3), ("HAPPY", 6)];

    for (word, expected) in tests {
        assert_eq!(
            MinimumDistance::minimum_distance(word.to_string()),
            expected
        );
    }
}
