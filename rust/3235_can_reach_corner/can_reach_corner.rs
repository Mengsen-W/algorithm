...
impl Solution {
    pub fn can_reach_corner(x_corner: i32, y_corner: i32, circles: Vec<Vec<i32>>) -> bool {
        ...
        fn circle_intersects_bottom_right(
            x: i32,
            y: i32,
            r: i32,
            x_corner: i32,
...
fn main() {
    let tests = vec![
        (3, 4, vec![vec![2, 1, 1]], true),
        (3, 3, vec![vec![1, 1, 2]], false),
        (3, 3, vec![vec![2, 1, 1], vec![1, 2, 1]], false),
        (4, 4, vec![vec![5, 5, 1]], true),
    ];

    for (x, y, circles, ans) in tests {
        assert_eq!(Solution::can_reach_corner(x, y, circles), ans);
    }
}
