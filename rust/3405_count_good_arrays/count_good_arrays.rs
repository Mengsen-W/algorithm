...
impl Solution {
    pub fn count_good_arrays(n: i32, m: i32, k: i32) -> i32 {
        init();
        (comb((n - 1) as usize, k as usize) * m as i64 % MOD
            * qpow((m - 1) as i64, (n - k - 1) as i32)
            % MOD) as i32
    }
}

fn main() {
    let tests = vec![(3, 2, 1, 4), (4, 2, 2, 6), (5, 2, 0, 2)];

    for (n, m, k, ans) in tests {
        assert_eq!(Solution::count_good_arrays(n, m, k), ans);
    }
}
