...
func getResults(queries [][]int) []bool {
	...
	for _, q := range queries {
		if q[0] == 1 {
			...
			maxSpace := max(x-pre, query(0, pre, 1, 0, mx))
...
func main() {
	tests := []struct {
		...
		{[][]int{{1, 2}, {2, 3, 3}, {2, 3, 1}, {2, 2, 2}}, []bool{false, true, false}},
		{[][]int{{1, 7}, {2, 7, 6}, {1, 2}, {2, 7, 5}, {2, 7, 6}}, []bool{true, true, false}},
	}

	for _, test := range tests {
		if res := getResults(test.queries); !reflect.DeepEqual(res, test.ans) {
