...
func main() {
	tests := []struct {
		...
		{3, [][]int{{0, 1, 2, 1}, {1, 2, 3, 0}}, 1, 2},
		{3, [][]int{{0, 1, 4, 0}, {1, 2, 3, 0}, {0, 2, 1, 0}}, 2, 6},
		{3, [][]int{{0, 1, 1, 1}, {1, 2, 1, 1}, {2, 0, 1, 1}}, 0, -1},
	}

	for _, test := range tests {
		if got := maxStability(test.n, test.edges, test.k); got != test.ans {
			fmt.Printf("test %v failed: got %v, want %v\n", test, got, test.ans)
		} else {
			fmt.Printf("test %v passed: got %v\n", test, got)
		}
	}
}
