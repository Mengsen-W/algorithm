...
func main() {
	tests := []struct {
		...
		{[][]int{{0, 0, 0, 0, 0}, {0, 0, 3, 0, 0}, {0, 1, 0, 0, 0}, {5, 0, 0, 3, 0}, {0, 0, 0, 0, 2}}, 11},
		{[][]int{{10, 9, 0, 0, 15}, {7, 1, 0, 8, 0}, {5, 20, 0, 11, 0}, {0, 0, 0, 1, 2}, {8, 12, 1, 10, 3}}, 94},
	}

	for index, test := range tests {
		if ans := maximumScore(test.grid); ans != int64(test.ans) {
			fmt.Printf("test %d failed: %d != %d\n", index, ans, test.ans)
		}
	}
}
