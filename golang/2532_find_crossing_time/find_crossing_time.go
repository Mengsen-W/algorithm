 ...
 * @FilePath: /algorithm/golang/2532_find_crossing_time/find_crossing_time.go
 */
...
func main() {
	testMap := []struct {
		...
		{1, 3, [][]int{{1, 1, 2, 1}, {1, 1, 3, 1}, {1, 1, 4, 1}}, 6},
		{3, 2, [][]int{{1, 9, 1, 8}, {10, 10, 10, 10}}, 50},
	}

	for _, item := range testMap {
		assert.Equal(&testing.T{}, findCrossingTime(item.n, item.k, item.time), item.ans)
	}
}
