...
func main() {
	{
		...
		assert.Equal(&testing.T{}, router.AddPacket(1, 4, 90), true)
		assert.Equal(&testing.T{}, router.AddPacket(2, 5, 90), true)
		assert.Equal(&testing.T{}, router.AddPacket(1, 4, 90), false)
		assert.Equal(&testing.T{}, router.AddPacket(3, 5, 95), true)
		assert.Equal(&testing.T{}, router.AddPacket(4, 5, 105), true)
		assert.Equal(&testing.T{}, router.ForwardPacket(), []int{2, 5, 90})
		assert.Equal(&testing.T{}, router.AddPacket(5, 2, 110), true)
		assert.Equal(&testing.T{}, router.GetCount(5, 100, 110), 1)
	}

	{
		router := Constructor(2)
		assert.Equal(&testing.T{}, router.AddPacket(7, 4, 90), true)
		assert.Equal(&testing.T{}, router.ForwardPacket(), []int{7, 4, 90})
		assert.Equal(&testing.T{}, router.ForwardPacket(), []int{})
	}
}
