 ...
 * @LastEditTime: 2021-07-13 09:10:46
 */
...
fn main() {
    {
        ...
        .to_vec();
        assert_eq!(get_skyline(buildings), ans);
    }
    {
        let buildings: Vec<Vec<i32>> = [[0, 2, 3].to_vec(), [2, 5, 3].to_vec()].to_vec();
        let ans: Vec<Vec<i32>> = [[0, 3].to_vec(), [5, 0].to_vec()].to_vec();
        assert_eq!(get_skyline(buildings), ans);
    }
}
